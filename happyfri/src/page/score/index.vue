<template>
  	<div>
    	<div class="your_scores_container">
            <header class="your_scores"><span class="score_num">{{this.score}}</span><span class="fenshu">分！</span></header>
            <div class="result_tip">{{this.scoreTips}}</div>
        </div>
        <div class="share_button" @click="showShare"></div>
        <div class="share_code">
            <header class="share_header">关注葡萄之家，获取答案。</header>
            <img src="../../images/4-4.png" height="212" width="212" class="code_img"> 
        </div>
        <div @click="showShare" class="share_cover" v-show="showHide">
...
export default {
    ...
    methods:{
        computedScore(){
            this.answerid.map(item=>{
                if(this.rightAnswer.indexOf(item)>-1){
                    this.score+=20;
                }
            });
